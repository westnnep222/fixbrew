...
class Glibc_lib238 < Package
  ...
  def self.install
    puts 'Installing Glibc_build238 to pull files for build...'.lightblue
    @filelist_path = File.join(CREW_META_PATH, 'glibc_build238.filelist')
    abort 'File list for Glibc_build238 does not exist! Please run: crew reinstall glibc_build238'.lightred unless File.file?(@filelist_path)
